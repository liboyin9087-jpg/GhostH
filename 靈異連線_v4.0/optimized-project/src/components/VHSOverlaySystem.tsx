...
interface VHSOverlaySystemProps {
  ...
  enhancedMode?: boolean;   // 增強模式（用於標題場景）
}
...
export const VHSOverlaySystem = memo(function VHSOverlaySystem({
  ...
  const effects = useMemo(() => {
    ...
    const enhanced = enhancedMode ? 0.15 : 0;
    
    return {
      // 雜訊透明度：基礎 3% + 強度加成（最高 15%）
      noiseOpacity: 0.03 + i * 0.12 + enhanced,
      // 掃描線透明度（增強：更密集）
      scanlineOpacity: 0.12 + i * 0.18 + enhanced * 0.1,
      // 掃描線間距（增強：每 2px）
      scanlineGap: enhancedMode ? 2 : 4,
      // 暈影強度
      ...
      // 綠色調強度
      greenTint: 0.06 + i * 0.08 + enhanced * 0.02,
      // 色差偏移量
      chromaticOffset: 2 + i * 2 + (enhancedMode ? 1 : 0),
      // 色差透明度
      chromaticOpacity: 0.06 + i * 0.04 + enhanced * 0.02,
    };
  }, [intensity01, phase, enhancedMode]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setHueShift(prev => {
        ...
        return Math.max(-15, Math.min(5, prev + delta));
      });
  ...
  // 隨機靜電干擾（每 8-15 秒）
  useEffect(() => {
    if (reducedMotion || !enhancedMode) return;
    
    const triggerStatic = () => {
      if (Math.random() < 0.2) {
        setStaticBurst(true);
        setTimeout(() => setStaticBurst(false), 200 + Math.random() * 300);
      }
    };
    
    const interval = setInterval(triggerStatic, 8000 + Math.random() * 7000);
    return () => clearInterval(interval);
  }, [reducedMotion, enhancedMode]);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-black">
      ...
      <div
        ...
        style={{
          filter: `${effects.colorFilter} hue-rotate(${hueShift}deg)`,
          transform: isIncident ? `translateX(${trackingOffset}px)` : undefined,
          transition: isIncident ? "none" : "transform 0.1s ease-out, filter 2s ease-in-out",
        }}
      >
        {children}
      </div>
      ...
      {!reducedMotion && (
        <>
          <div
            aria-hidden
            className="absolute inset-0 z-[82] pointer-events-none mix-blend-screen"
            style={{
              background: `rgba(255, 0, 0, ${effects.chromaticOpacity})`,
              transform: `translateX(${effects.chromaticOffset}px)`,
            }}
          />
          <div
            aria-hidden
